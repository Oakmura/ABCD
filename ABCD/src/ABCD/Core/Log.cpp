...
namespace abcd
{
    ...
    void Log::Init()
    {
        ...
        logSinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("ABCD.log", true));

        logSinks[0]->set_pattern("%^[%T] %n: %v%$");
        logSinks[1]->set_pattern("[%T] [%l] %n: %v");

        sCoreLogger = std::make_shared<spdlog::logger>("ABCD", begin(logSinks), end(logSinks));
